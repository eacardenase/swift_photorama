...
class PhotoStore {
    ...
    private func processPhotosRequest(
        data: Data?,
        error: Error?,
        completion: @escaping (Result<[Photo], Error>) -> Void
    ) {
    ...
    private func processImageRequest(data: Data?, error: Error?) -> Result<
        UIImage, Error
    > {
    ...
    func fetchAllPhotos(completion: @escaping (Result<[Photo], Error>) -> Void)
    {
        ...
        let sortByDateTaken = NSSortDescriptor(
